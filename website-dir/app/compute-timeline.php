...
$query = mysqli_query(establishDB(), "SELECT * FROM p_post left join f_favorites f2 on p_post.p_id = f2.f_p_post WHERE f_u_user is not ".$_SESSION["u_user"]->u_id." && p_u_user is not ".$_SESSION["u_user"]->u_id." LIMIT ".$count.";");
if (mysqli_num_rows($query) > 0) {
    ...
    if(isset($_REQUEST["method"]) && $_REQUEST["method"] == "thrift-it"){
        echo "<div class=\"thrift-it d-flex justify-content-center align-items-center flex-column \"><h2 class='text-danger'>Keine Produkte Mehr Vorhanden!</h2><h4 class='text-success'> Schaue später noch einmal vorbei :)</h4></div>";
    }else{
